...
library(pedsuite)
Tutankhamun = readPed("data-raw/tutankhamun-jama.txt", colSep = "\t")
plot(Tutankhamun)
usethis::use_data(Tutankhamun, overwrite = TRUE)


#### Testing
showInTriangle(ibdEstimate(Tutankhamun)[11:6,], labels = T)

t1 = reconstruct(Tutankhamun, ids = labels(Tutankhamun)[1:4],
                 noChildren = labels(Tutankhamun)[3:4], linear = FALSE, extra = 2)
plot(t1, 6)
